...
class VisualizationDemo(object):
    def __init__(self, cfg, instance_mode=ColorMode.IMAGE, parallel=False, batch_mode = False):
        """
        Args:
            cfg (CfgNode):
    ...
    def inference_on_image(self, image):
        ...
        """
        if self.batch_mode:
            p_l, pts_l = self.inference_on_batch([image])
            return p_l[0], pts_l[0]
    ...
    def inference_on_batch(self, images):
        predictions_list = self.predictor(images)
        ...
        poly_text_score_dict_list_list = []
        for predictions in predictions_list:
            assert "instances" in predictions
            instances = predictions["instances"].to(self.cpu_device)
            poly_text_score_dict_list = visualizer.get_human_readable_prediction(predictions=instances)
            poly_text_score_dict_list_list.append(poly_text_score_dict_list)
            del predictions["instances"]

        return None, poly_text_score_dict_list_list
...
class BatchPredictor:
    ...
    def __call__(self, original_images):
        ...
        with torch.no_grad():  # https://github.com/sphinx-doc/sphinx/issues/4258
            ...
            predictions = self.model(inputs_lst)[:num_images]
